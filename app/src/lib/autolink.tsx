...
export function buildLinkIndex(begriffe: Fachbegriff[]): LinkIndex {
  ...
  // Frontières de mot en tenant compte des lettres allemandes.
...
export function AutoLinkText({ text, index, onOpen }: AutoLinkTextProps) {
  const parts = useMemo(() => {
    if (!index.regex || !text) return [{ t: text, fb: null as Fachbegriff | null }];
    const out: { t: string; fb: Fachbegriff | null }[] = [];
    let last = 0;
