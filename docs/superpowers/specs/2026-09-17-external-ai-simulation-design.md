...
| D7 | **Fidélité avant concision** : le prompt garde toute la persona, toutes les répliques du Rollenskript, toutes les questions de l'Oberarzt ; borne dure `PROMPT_MAX = 56000` (corpus fidèle mesuré : médiane 31,8 k, max 44,2 k ≈ 11 k tokens — collable partout) avec compaction *non destructive* seulement au-delà (retrait des attendus « (erwartet: …) », puis répliques → Fakten sur les chapitres secondaires) ; les « Fakten » ne sont rendus que pour les chapitres sans répliques (dédoublonnage permanent) ; sur les signes niés, la raison différentielle « (gegen …) » est retirée — c'est la fiche médicale. Jamais de troncature de texte. Le diagnostic apparaît **seulement** dans la section Oberarzt, avec la consigne « als Patient kennst du diese Diagnose nicht » — pas de caviardage | 109/130 cas dépassent 6 000 car. en version complète ; un patient tronqué n'est plus le cas |
...
export const PROMPT_MAX = 56000;   // borne dure (presse-papiers) — corpus fidèle : max 44,2 k
export function buildExternalPromptDetailed(i: PromptInput): { text: string; level: 'full' | 'a' | 'ac' }
```
...
6. **Start** — « Antworte auf diese Nachricht nur mit „Bereit.“ und warte auf die Begrüßung. Dann stell dich mit einem Satz vor. Nenne nie eine Diagnose. Sprachmodus empfohlen. » (le prompt est le premier message — l'IA ne doit pas le commenter)

Invariants testés : contient chaque `antwort` du Rollenskript et toute la `persona` ; aucune chaîne de `medicalView` (≥ 12 car., hors chaînes aussi présentes dans `patientSheet`) **avant** le marqueur « # Teil 3 – Oberarzt/Oberärztin » (la verdachtsdiagnose peut apparaître dans les questions de l'Oberarzt, jamais dans les sections patient) ; `level === 'full'` et `length ≤ PROMPT_MAX` pour les 130 cas avec 8 `topTerms` réalistes ; scope `anamnese` ne contient pas « Fallvorstellung » ; part des cas ≤ `PREFILL_MAX` = 0 % (mesuré) : le chemin normal est **ouverture + presse-papiers**, le pré-remplissage reste un bonus si un prompt court apparaît.
...
| AC-3 | 130/130 cas : niveau `full` (aucune compaction) et `length ≤ 56000` de persona/répliques/questions ; rapport : part des cas ≤ 6 000 (pré-remplissables) | test corpus |
...
| Prompt > 6 000 (tous les cas) | presse-papiers + « colle-le et envoie » est le flux normal (un geste de plus, aucune perte) ; la feuille l'annonce clairement ; compaction non destructive au-delà de 56 000 (aucun cas aujourd'hui ; le test corpus l'assert) |
